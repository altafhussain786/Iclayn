...
// const ContigencyFeeDetails = ({ item, navigation }) => {
...
// export default ContigencyFeeDetails;
...
const ContingencyFeeDetails = ({ item, navigation }) => {
    ...
    const handleRemoveItem = () => {
        dispatch(removeContingencyFee({ id }));
    };

    console.log("item consitengiddf =======================>", item);
        ...
        <View style={{ borderBottomWidth: 0.5, borderColor: COLORS?.BORDER_LIGHT_COLOR, paddingBottom: 15 }}>
            ...
            <View style={{ marginTop: 10 }}>
                <TextInput
                    placeholder="Description"
                    value={String(item?.description || '')}
                    onChangeText={(txt) => dispatch(updateContingencyFeeField({ id, field: 'description', value: txt }))}
                    style={styles.input}
                />
            </View>
            ...
            <View style={{ marginTop: 10 }}>
                <TextInput
                    ...
                    keyboardType='numeric'
                    value={String(item?.awardedAmount || '')}
                    onChangeText={(txt) => dispatch(updateContingencyFeeField({ id, field: 'awardedAmount', value: txt }))}
                    style={styles.input}
                />
            </View>
            ...
            <View style={{ marginTop: 10 }}>
                <TextInput
                    ...
                    keyboardType='numeric'
                    value={String(item?.contingencyRate || '')}
                    onChangeText={(txt) => dispatch(updateContingencyFeeField({ id, field: 'contingencyRate', value: txt }))}
                    style={styles.input}
                />
            </View>
            ...
            <View style={{ alignItems: "flex-end" }}>
                <View style={{ marginVertical: 10 }}>
                    {(() => {
                        ...
                        const contingencyRate = Number(item?.contingencyRate || 0); // percentage
                        ...
                        return (
                            <>
                                <MyText style={styles.totalTaxt}>Amount : {amount}</MyText>
                                <MyText style={styles.totalTaxt}>Tax Amount : {taxTotal}</MyText>
                                <MyText style={styles.totalTaxt}>Total : {total}</MyText>
                            </>
                        );
                    })()}
                </View>
            ...
            <BottomModalListWithSearch
                ...
                renderItem={({ item: option }) => (
                    <TouchableOpacity
                        onPress={() => {
                            dispatch(updateContingencyFeeField({ id, field: 'userObj', value: option }));
                            dispatch(updateContingencyFeeField({ id, field: 'user', value: option.userProfileDTO?.fullName }));
                            setisOpenUser(false);
                        }}
            ...
            <DatePicker
                ...
                date={new Date()}
                onConfirm={date => {
                    dispatch(updateContingencyFeeField({ id, field: 'date', value: moment(date).format('MM/DD/YYYY') }));
                    setIsDateOpen(false);
                }}
            ...
            <BottomModalListWithSearch
                ...
                renderItem={({ item: option }) => (
                    <TouchableOpacity
                        onPress={() => {
                            dispatch(updateContingencyFeeField({ id, field: 'tax', value: option.name }));
                            dispatch(updateContingencyFeeField({ id, field: 'taxObj', value: option }));
                            dispatch(updateContingencyFeeField({ id, field: 'taxAmount', value: option?.rate }));
                            setisOpenTax(false);
                        }}
                        style={styles.modalItem}
                    >
...
export default ContingencyFeeDetails;
