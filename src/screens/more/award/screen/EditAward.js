...
const EditAward = ({ navigation, route }) => {
    ...
    const getDefaultData = async () => {
        const { res, err } = await httpRequest({
            method: `get`,
            path: `/ic/matter/award/${communicationDetails?.matterAwardId}`,
        ...
        if (res) {
            ...
            console.log(err, "GET CUSTOMER RESdPONSE===>err");
        }
    }
        ...
        <>
            <Formik
                ...
                initialValues={
                    {
                        ...
                        //Firm Data
                        firmData: firmData?.find(user => user?.userId === defaultData?.feeRecipientId)?.userProfileDTO?.fullName || "",
                        firmObj: firmData?.find(user => user?.userId === defaultData?.feeRecipientId) || {},
                        isOpenfirm: false,
                        // firmItems: [],
                ...
                onSubmit={async (values, { setFieldValue }) => {
                    ...
                    const payload =
                    {
                        ...
                        totalAmount: String(defaultData?.totalAmount),
                        matterAwardId: defaultData?.matterAwardId || null,
                        matterName: values?.matterObj?.name || null,
                        awardDate: values?.selectedDate,
                        feeRecipientId: values?.firmObj?.userId,
                    ...
                    console.log(payload, "PAYLOAD", defaultData);
                    ...
                    if (res) {
                        toast.show('Award updated successfully', { type: 'success' })
            ...
            >
                ...
                {({ handleChange, handleBlur, handleSubmit, values, errors, touched, setFieldValue }) => (
                    ...
                    <>
                        <ScreenHeader isLoading={values?.loader} onPressSave={handleSubmit} isShowSave={true} extraStyle={{ backgroundColor: '#F5F6F8' }} isGoBack={true} onPress={() => { navigation.goBack() }} isShowTitle={true} title="Edit Award" />
                        <Wrapper>
                            <ScrollView
